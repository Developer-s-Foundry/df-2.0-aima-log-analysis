...
async def check_rate_limit_dependency(request: Request):
    ...
    allowed, info = await rate_limiter.check_rate_limit(
        identifier=identifier,
        max_requests=settings.rate_limit_requests if hasattr(settings, 'rate_limit_requests') else 100,
        window_seconds=settings.rate_limit_period if hasattr(settings, 'rate_limit_period') else 60
    )
